...
var r="";
if(o&&o.length){
var n=1e3,a=o.length,s=Math.ceil(a/n);
r=["&lc="+s];
for(var i=0;s>i;++i)r.push("&log"+i+"=[rtCheckError]["+i+"]"+encodeURIComponent(o.substr(i*n,n)));
r=r.join("");
}
...
var n=1e3,a=o.length,s=Math.ceil(a/n);
r=["&lc="+s];
for(var i=0;s>i;++i)r.push("&log"+i+"=[Ajax Length Limit]["+i+"]"+encodeURIComponent(o.substr(i*n,n)));
r=r.join("");
}
var c,p="idkey="+e+"_"+t+"_1"+r+"&r="+Math.random();
if(window.ActiveXObject)try{
...
var o=K[e],i=_.attr(o,"data-showed");
if("no"==i){
...
function a(e){
try{
for(var i=window.pageYOffset||document.documentElement.scrollTop,a=0;a<K.length;a+=S){
var m=K[a];
if(!(m.offsetTop>i+k.getInnerHeight()+100)){
var l=_.attr(m,"data-cpsstatus");
if("hide"==l){
m.setAttribute("data-cpsstatus","loading");
for(var w=""+a,u=1,f=a+1;f<K.length&&a+S>f;f++)w=w+"%2c"+f,u++;
...
source_logo_url:"",
is_ad:1
},P=_.attr(K[D],"data-templateid");
"list"==P?K[D].innerHTML=r.tmpl(c,W):"banner"==P?K[D].innerHTML=r.tmpl(s,W):"card"==P&&(K[D].innerHTML=r.tmpl(d,W));
}
}catch(H){
console.log(H);
}
...
document.title="",L=!1,T(),window.onscroll=T,window.addEventListener("pageshow",T);
});
}));
...
var _t=Math.random();
if(2e-4>_t)try{
for(var ut=document.getElementsByTagName("img"),ft=window.screen.height,ht=window.screen.width,vt=0,yt=window.devicePixelRatio,vt="",D=0,bt=ut.length;bt>D;D++){
var At=ut[D].getAttribute("data-src");
if(At){
var jt=ut[D].getBoundingClientRect();
